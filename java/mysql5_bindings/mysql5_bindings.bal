...
function urlOf(Conf c) returns string =>
    string `jdbc:mysql://${c.host}:${c.port}/${c.database}`;

// Ouvre la connexion et retourne notre Client
public function connect(Conf conf) returns Client|error {
...
public function update(Client c, string sql) returns int|error {
    return check c.impl.executeUpdate(sql);
}

// Exécute SELECT et retourne directement un `json`
public function query(Client c, string sql) returns json|error {
